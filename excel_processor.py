    ...
    for row in range(2, ws.max_row + 1):
        try:
            ...
            if i_value:
                ...
                elif "BPH-041" in i_value and any(kw in i_value for kw in ["여분필터", "여분 필터", "여분필터 추가", "여분 필터 추가"]):
                    q_cell.value = "4wb_v1.5"
                elif "BPH-041" in i_value and not any(kw in i_value for kw in ["여분필터", "여분 필터", "여분필터 추가", "여분 필터 추가"]):
                    q_cell.value = "4wa_v1.5"
                elif "BPH-011" in i_value and ("블랙" in i_value or "Black" in i_value) and any(kw in i_value for kw in ["여분필터", "여분 필터", "여분필터 추가", "여분 필터 추가"]):
                    q_cell.value = "kb_v1.5"
                elif "BPH-011" in i_value and ("블랙" in i_value or "Black" in i_value):
                    q_cell.value = "ka_v1.5"
                elif "BPH-011" in i_value and ("화이트" in i_value or "White" in i_value) and any(kw in i_value for kw in ["여분필터", "여분 필터", "여분필터 추가", "여분 필터 추가"]):
                    q_cell.value = "wb_v1.5"
                elif "BPH-011" in i_value and ("화이트" in i_value or "White" in i_value):
            ...
            n_value = str(n_cell.value).strip() if n_cell.value else ""

            print(f"🔎 N열 원본값: '{n_value}'")
